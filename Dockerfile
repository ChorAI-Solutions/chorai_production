...
CMD ["sh", "-c", "node ./scripts/debug-log-proxy.mjs >/dev/null 2>&1 & npx prisma migrate deploy --schema=prisma/schema.prisma || echo 'Prisma migrate skipped (will not block app start)'; node server.js"]
